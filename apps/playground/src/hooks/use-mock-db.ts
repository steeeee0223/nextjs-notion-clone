...
export const useMockDB = () => {
      ...
      dBAction(
        () =>
          update((prev) => {
            ...
            return {
              ...prev,
              [collection]: (prev[collection] as Model<K>[]).map((model) =>
                where(model) ? { ...(model as object), ...data } : model,
              ),
            };
          }),
        (e) => console.error(`[db:${collection}] update error`, e),
  ...
  const findWorkspaceMemberships = useCallback(
    (workspaceId: string) =>
      actions.findWorkspaceMemberships(
        {
